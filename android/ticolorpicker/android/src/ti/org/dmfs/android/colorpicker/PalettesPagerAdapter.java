 ...
 */
public class PalettesPagerAdapter extends FragmentStatePagerAdapter implements
		IDrawableTitlePagerAdapter {
	...
	public PalettesPagerAdapter(Resources res, FragmentManager fm,
		...
		protected Drawable create(Integer key) {
			...
			for (int j = 0, k = palette.getNumberOfColors() / cols; j < k; ++j) {
				for (int i = 0, l = cols; i < l; ++i) {
					...
					// canvas.drawRect(i * grid, j * grid, (i + 1) * grid -
					// spacing, (j + 1) * grid - spacing, paint);
					canvas.drawCircle((i + 0.5f) * grid - halfSpacing,
							(j + 0.5f) * grid - halfSpacing, radius, paint);
				}
			}
